...
{
    /*
    uploadPath : 파일을 지정할 위치(파일 시스템)
    originalFileName : 유저가 첨부한 파일의 원래 이름
    fileDate : 파일의 바이너리 데이터(원본)

    <<purpose>>
    파일을 파일시스템에 저장
    originalFileName 그대로 저장하면 중복(덮어쓰기) 위험
    originalFileName ==> 중복 위험에 안전한(UUID(규격이 있는 난수)를 적용한)이름으로 변경해서 저장
     */
    public String uploadFile(String uploadPath, String originalFileName,
                             byte[] fileDate) throws Exception {
        /* 1. originalFilename ==> UUID가 적용된 이름으로 변경
        1.UUID생성
        2.originalFilename에서 파일 확장자 열기
        */
